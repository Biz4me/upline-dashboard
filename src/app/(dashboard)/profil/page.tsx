...
import { Edit2, Check, User, Mail, Phone, MapPin, Globe, FileText, Trophy, Settings, CreditCard, Sparkles, Camera, Link, MessageCircle, Bell, Lock, Trash2, ChevronRight } from 'lucide-react'
...
export default function ProfilPage() {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 64px)', overflow: 'hidden', background: 'var(--bg-page)' }}>
      ...
      {/* ===== SIDEBAR GAUCHE ===== */}
      <div style={{ width: 240, flexShrink: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflowY: 'auto', background: 'var(--bg-card)' }}>
        ...
        <div style={{ padding: '28px 20px 20px', borderBottom: '1px solid var(--border)', textAlign: 'center' }}>
          <div style={{ position: 'relative', width: 72, height: 72, margin: '0 auto 12px' }}>
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg, #6D5EF5, #22D3EE)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 800, color: 'white', boxShadow: '0 4px 16px rgba(109,94,245,0.35)' }}>
              {initials}
            </div>
            <button style={{ position: 'absolute', bottom: 0, right: 0, width: 24, height: 24, borderRadius: '50%', background: '#6D5EF5', border: '2px solid var(--bg-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              <Camera size={11} color="white" />
            </button>
          </div>
          <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)', marginBottom: 4 }}>
            {profile.prenom || session?.user?.name?.split(' ')[0] || 'Prénom'} {profile.nom || session?.user?.name?.split(' ')[1] || ''}
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
            @{session?.user?.name?.toLowerCase().replace(' ', '') || 'utilisateur'}
          </div>
          {profile.societe && (
            <div style={{ fontSize: 11, fontWeight: 700, background: 'rgba(34,211,238,0.1)', color: '#22D3EE', padding: '3px 10px', borderRadius: 20, display: 'inline-block', marginBottom: 12 }}>
              {profile.societe}
            </div>
          )}
          {/* Score */}
          <div style={{ background: 'var(--bg-page)', borderRadius: 12, padding: '10px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Profil complété</span>
              <span style={{ fontSize: 13, fontWeight: 800, color: score >= 80 ? '#22C55E' : score >= 50 ? '#6D5EF5' : '#FF9600' }}>{score}%</span>
            </div>
            <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ width: `${score}%`, height: '100%', background: 'linear-gradient(135deg, #6D5EF5, #22D3EE)', borderRadius: 3, transition: 'width 0.5s' }} />
            </div>
          </div>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isActive ? 'rgba(109,94,245,0.1)' : 'transparent',
                  color: isActive ? '#6D5EF5' : 'var(--text-secondary)',
                  fontSize: 13, fontWeight: isActive ? 700 : 500,
                  cursor: 'pointer', textAlign: 'left', marginBottom: 2,
                  transition: 'all 0.15s',
                  borderLeft: isActive ? '3px solid #6D5EF5' : '3px solid transparent',
                }}
                onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'var(--primary-muted)' }}
        ...
        <div style={{ padding: '12px 10px', borderTop: '1px solid var(--border)' }}>
          <button
            ...
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, border: 'none', background: 'transparent', color: '#EF4444', fontSize: 13, fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {['Marché chaud', 'Marché tiède', 'Marché froid', 'Réseaux sociaux', 'Événements'].map(m => (
                    <button key={m} style={{ background: 'rgba(109,94,245,0.1)', border: '1.5px solid rgba(109,94,245,0.25)', color: '#a78bfa', borderRadius: 20, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                      {m}
                    </button>
                  ))}
                </div>
        ...
        {tab === 'parametres' && (
          <>
            ...
            <Section title="Confidentialité" icon={Lock}>
              <Toggle label="Profil public dans la communauté" desc="Les autres membres peuvent vous voir" value={true} onChange={() => {}} />
              <Toggle label="Statistiques visibles" desc="Votre progression dans le classement" value={false} onChange={() => {}} />
            </Section>
          ...
          <>
            <Section title="Mon abonnement" icon={CreditCard}>
              ...
              {['Gérer mon abonnement', 'Historique des paiements', 'Mettre à jour ma carte'].map((item, i) => (
                <button key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)', padding: '14px 0', cursor: 'pointer', color: 'var(--text)', fontSize: 14, fontWeight: 500 }}
            ...
            <Section title="Sécurité" icon={Lock}>
              <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)', padding: '14px 0', cursor: 'pointer', color: 'var(--text)', fontSize: 14, fontWeight: 500 }}
                onMouseEnter={e => e.currentTarget.style.color = '#6D5EF5'}
                onMouseLeave={e => e.currentTarget.style.color = 'var(--text)'}
              >
